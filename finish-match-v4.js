(() => {
  ...
  async function ensureTargetDepth(target, onProgress) {
    if (!target) return null;
    if (target.depthCanvas && target.rawDepth) {
      return { canvas: target.depthCanvas, rawDepth: target.rawDepth };
    }
    if (!window.DepthEngine) {
      throw new Error('Depth Anything engine is not loaded. Please ensure depth-engine.js is present.');
    }
    try {
      const res = await window.DepthEngine.estimateDepth(target.img, onProgress);
      target.depthCanvas = res.canvas;
      target.rawDepth = res.rawDepth;
      target.depthData = res.imageData;
      const engine = (typeof ColorEngine !== 'undefined' ? ColorEngine : (typeof window !== 'undefined' ? window.ColorEngine : null));
      if (engine && engine.computeDepthGradients) {
        target.depthGradients = engine.computeDepthGradients(res.rawDepth, res.width, res.height);
      }
      return res;
    } catch (err) {
      console.error('[DepthEngine] Depth estimation failed:', err);
      throw err;
    }
  }

  async function estimateCurrentTargetDepth() {
    const target = activeTarget();
    if (!target) return;
    const btn = refs.estimateDepthBtn;
    ...
    refs.batchProgressBar.style.width = '10%';
    refs.batchProgressText.textContent = 'Loading Depth Anything model…';

    try {
      await ensureTargetDepth(target, (pct, text) => {
        refs.batchProgressBar.style.width = `${pct}%`;
        refs.batchProgressText.textContent = text;
      });
      target.tuned = true;
      refs.resultNote.textContent = `AI Depth map calculated for ${target.name}. Local depth now utilizes monocular 3D spatial layers.`;
      drawPreview();
    } catch (err) {
      alert(`Depth Estimation Failed: ${err.message}`);
    } finally {
      if (btn) {
        btn.disabled = false;
        btn.textContent = oldText;
      }
      setTimeout(() => refs.batchProgress.classList.add('hidden'), 1500);
    }
  }

  async function open3DDepthViewer() {
    const target = activeTarget();
    if (!target) return;
    refs.depthModal3D.classList.remove('hidden');
    refs.batchProgress.classList.remove('hidden');
    refs.batchProgressBar.style.width = '15%';
    refs.batchProgressText.textContent = 'Calculating 3D Depth Map…';
    ...
    try {
      ...
      let depthSourceCanvas = target.depthCanvas;
      const colormap = refs.depthColormapSelect ? refs.depthColormapSelect.value : 'rgb';
      if (colormap !== 'rgb' && window.DepthEngine) {
        depthSourceCanvas = window.DepthEngine.colorizeDepth(target.depthCanvas, colormap);
      }
      ...
      active3DViewer = window.DepthEngine.create3DViewer(
        refs.depth3DContainer,
        target.img,
        depthSourceCanvas,
        {
          pointSize: +refs.pointSizeSlider.value,
          depthScale: +refs.depthScaleSlider.value
        }
      );
    } catch (err) {
      alert(`3D Depth Viewer Error: ${err.message}`);
      refs.depthModal3D.classList.add('hidden');
    } finally {
      setTimeout(() => refs.batchProgress.classList.add('hidden'), 1200);
    }
  }

  function close3DDepthViewer() {
    refs.depthModal3D.classList.add('hidden');
    if (active3DViewer) {
      active3DViewer.destroy();
      active3DViewer = null;
  ...
  async function exportTargetDepth() {
    const target = activeTarget();
    if (!target) return;
    const btn = refs.exportDepthBtn;
    const oldText = btn ? btn.textContent : '';
    if (btn) {
      btn.disabled = true;
      btn.textContent = 'Preparing Depth…';
    }
    try {
      await ensureTargetDepth(target);
      const blob = await new Promise((resolve) => target.depthCanvas.toBlob(resolve, 'image/png'));
      downloadBlob(blob, `${cleanBaseName(target.name)}_depthanything.png`);
      refs.resultNote.textContent = `Depth map exported for ${target.name}.`;
    } catch (err) {
      alert(`Failed to export depth map: ${err.message}`);
    } finally {
      if (btn) {
        btn.disabled = false;
        btn.textContent = oldText;
  ...
  refs.estimateDepthBtn?.addEventListener('click', estimateCurrentTargetDepth);
  refs.open3DBtn?.addEventListener('click', open3DDepthViewer);
  refs.close3DModalBtn?.addEventListener('click', close3DDepthViewer);
  refs.exportDepthBtn?.addEventListener('click', exportTargetDepth);

  refs.pointSizeSlider?.addEventListener('input', (e) => {
    if (refs.pointSizeVal) refs.pointSizeVal.value = e.target.value;
    if (active3DViewer && activeTarget()?.depthCanvas) {
      open3DDepthViewer();
    }
  });

  refs.depthScaleSlider?.addEventListener('input', (e) => {
    if (refs.depthScaleVal) refs.depthScaleVal.value = e.target.value;
    if (active3DViewer && activeTarget()?.depthCanvas) {
      open3DDepthViewer();
    }
  });

  refs.depthColormapSelect?.addEventListener('change', () => {
    if (active3DViewer && activeTarget()?.depthCanvas) {
      open3DDepthViewer();
    }
  });

  refs.reset3DCameraBtn?.addEventListener('click', () => {
    if (active3DViewer && active3DViewer.camera && active3DViewer.controls) {
      active3DViewer.camera.position.set(0, 0, 2.2);
